...
function process_form(){
	...
	if(!$qc){	print("Se ha producido un error: ".mysqli_error($db)."<br/><br/>");
		...
		<tr>
			<td colspan=2 align='center'>
				<img style='height:60px; width:45px;' src='../Img.User/".$rowqc['myimg']."'  />
			</td>
		</tr>
		<tr>
			<td colspan=2 align='center'>
...
function desbloqueo(){
	...
	*/
	...
	global $table_name_b;	$table_name_b = "`".$_SESSION['clave']."_ipcontrol`";
	$desb = "UPDATE `$db_name`.$table_name_b SET `ref` = '$_POST[refdesb]', `nivel` = '$_POST[nivdesb]', `error` = 'des', `acceso` = 'des' WHERE $table_name_b.`ipn` = '$uip' AND $table_name_b.`acceso` = 'x' OR $table_name_b.`acceso` = '0' ";
	...
	if(mysqli_query($db, $desb)){ $_SESSION['showf'] = 0;
			...
			print("<table align='center' style=\"margin-top:2px; margin-bottom:2px\" >
						<tr>
							<td align='center'>
								SU IP ".$uip." HA SIDO DESBLOQUEADA
							</td>
						</tr>
						<tr>
							<td align='center'>
