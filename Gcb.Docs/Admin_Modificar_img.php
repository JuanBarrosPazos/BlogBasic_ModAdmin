...
function process_form(){
	...
	$destination_file = '../Gcb.Img.User/'.$safe_filename;
	

	if( file_exists( '../Gcb.Img.User/'.$safe_filename) ){
		unlink('../Gcb.Img.User/'.$safe_filename);
			}
			
	elseif (move_uploaded_file($_FILES['myimg']['tmp_name'], $destination_file)){
		
		if( file_exists( '../Gcb.Img.User/'.$_SESSION['smyimg'])){
			unlink('../Gcb.Img.User/'.$_SESSION['smyimg']);
		}else{}
		...
		$rename_filename = "../Gcb.Img.User/".$new_name;	
